...
def select_operation(root, operations):
    ...
    tk.Label(root, text="Choose an operation").grid(row=1, column=1)
...
def main():
    ...
    root.mainloop()  # this will block the script until the root window is closed

    operation = root.operation  # retrieve the operation after the window is closed
